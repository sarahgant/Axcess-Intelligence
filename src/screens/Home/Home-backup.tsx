...
export const Home = (): JSX.Element => {
  ...
  // AI Model state using the useModels hook
  const {
    models,
    availableModels,
    selectedModel,
  ...
  // Update AI initialization state based on models loading
  useEffect(() => {
    setIsInitializingAI(isLoadingModels);
  }, [isLoadingModels]);
                          ...
                          <div className="flex items-center gap-1 mt-2 px-1 justify-end">
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                if (messageIndex !== -1 && currentConversationId) {
                                  setConversations(prev => prev.map(conv =>
                                    conv.id === currentConversationId
            ...
            <div className="flex flex-col items-start justify-center gap-2 px-4 py-3 self-stretch w-full bg-white border border-solid border-[#757575] relative">
              ...
              <div className="items-center justify-between self-stretch w-full flex relative">
                    ...
                    <button
                      ...
                      title={
                        isInitializingAI
                          ? 'Loading models...'
                          : selectedModel
                            ? `AI Model: ${getModelInfo(selectedModel)?.displayName || selectedModel}`
                            : isBackendConnected
                              ? 'Select AI Model'
                              : 'Backend not connected'
                      }
                      disabled={isInitializingAI || !isBackendConnected}
                    >
                      ...
                      <div className="absolute -top-0.5 -right-0.5 z-10">
                        {isInitializingAI ? (
                          ...
                          />
                        ) : isBackendConnected && selectedModel ? (
                      ...
                      />
                      {!isInitializingAI && availableModels.length > 1 && (
                        <img
                ...
                <div className="inline-flex items-center">
                  <button
                    onClick={handleSend}
                    disabled={isLoading || !inputText.trim() || isInitializingAI || !isBackendConnected || !selectedModel}
                    title={
                      isInitializingAI
                        ? 'Loading models...'
                        : !isBackendConnected
                    ...
                    className={`inline-flex items-center justify-center p-3 relative rounded transition-all disabled:cursor-not-allowed ${inputText.trim() && !isLoading && !isInitializingAI && isBackendConnected && selectedModel
